...
def prediction(model, test_loader,epoch, prec):
    model.eval()
    all_predictions = []
    ...
    submission.to_csv(f'./res_1_{epoch}_{prec}.csv', index=False)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            prediction(model, test_loader, epoch, prec1)
    ...
    prediction(model, test_loader, "final",  None)
