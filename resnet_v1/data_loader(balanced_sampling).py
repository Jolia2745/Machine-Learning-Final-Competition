import torch
from torch.utils.data import Dataset, DataLoader
import pickle
import os
from sklearn.model_selection import train_test_split

from collections import Counter
from sklearn.utils import resample


class PickleAudioDataset(Dataset):
    def __init__(self, file_paths, labels=None, transform = None): # transform = normalize_mel
        """
        file_paths: List of paths to the pickle files
        labels: Optional, pass labels for supervised learning
        transform: Optional, transformations to apply to the data
        """
        self.file_paths = file_paths
        self.labels = labels
        self.transform = transform

    def __len__(self):
        return len(self.file_paths)

    def __getitem__(self, idx):
        with open(self.file_paths[idx], 'rb') as f:
            feature = pickle.load(f)
            # print(111, type(feature))
        
        # Transform the feature if a transform is provided
        if self.transform:
            feature = self.transform(feature)
        
        # Convert numpy array to torch.Tensor and add a channel dimension
        feature = torch.tensor(feature, dtype=torch.float32).unsqueeze(0)  # Adds the channel dimension

        if self.labels is not None:
            label = self.labels[idx]
            return feature, label
        
        return feature
    
def normalize_mel(mel_spec):
    # <class 'data_loader.PickleAudioDataset'>
    epsilon = 1e-10  # smoothing
    mel_spec_mean = mel_spec.mean()
    mel_spec_std = mel_spec.std()
    
    # 确保标准差不为零
    if mel_spec_std == 0:
        mel_spec_std += epsilon
    
    mel_spec = (mel_spec - mel_spec_mean) / mel_spec_std
    return mel_spec

def load_labels(label_file):
    with open(label_file, "r") as f:
        return [int(label.strip()) for label in f]

def load_dataset_paths(root_dir, file_extension='.pkl'):
    files = [f for f in os.listdir(root_dir) if f.endswith(file_extension)]
    
    files_sorted = sorted(files, key=lambda x: int(x.split('.')[0]))           
    '''
        mel =  [os.path.join(root_dir, f) for f in sorted(os.listdir(root_dir)) if f.endswith(file_extension)]
        only sort the length!!
    '''
    mel = [os.path.join(root_dir, f) for f in files_sorted]
    
    return mel

def balance_dataset_by_oversampling(paths, labels):

    label_counter = Counter(labels)
    print(label_counter)

    # max_samples = max(label_counter.values())
    max_samples = 3000

    new_paths, new_labels = [], []
    for label, count in label_counter.items():
        if label == "0" or count > max_samples: # bc "0" has high accuracy
            current_paths =[path for path, l in zip(paths, labels) if l == label]
            new_paths.extend(current_paths)
            new_labels.extend([label] * len(current_paths))
            continue


        current_paths = [path for path, l in zip(paths, labels) if l == label]
        if 0 == max_samples - count:
            print(1)
            continue

        resampled_paths = resample(current_paths, replace=True, n_samples=max_samples - count)
        new_paths.extend(current_paths + resampled_paths)
        new_labels.extend([label] * (max_samples))
    return new_paths, new_labels

def create_dataloaders(train_dir, test_dir, label_file, batch_size=32, test_batch_size=32, val_size=0.2, shuffle=True):
    images_paths = load_dataset_paths(train_dir)
    labels = load_labels(label_file)
    # print(len(images_paths))  11886
    
    # balanced_train_paths, balanced_train_labels = balance_dataset_by_oversampling(images_paths, labels)

    test_paths = load_dataset_paths(test_dir)

    train_paths, val_paths, train_labels, val_labels = train_test_split(  images_paths , labels,test_size=val_size, random_state=42)

    balanced_train_paths, balanced_train_labels = balance_dataset_by_oversampling( train_paths, train_labels) # ({3: 3485, 2: 2758, 1: 2424, 0: 841})

    train_dataset = PickleAudioDataset(balanced_train_paths, balanced_train_labels, normalize_mel)
    val_dataset = PickleAudioDataset(val_paths, val_labels, normalize_mel)
    # print(val_dataset[0][0].size())
    test_dataset = PickleAudioDataset(test_paths, None, normalize_mel)

    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
    test_loader = DataLoader(test_dataset, batch_size=test_batch_size, shuffle=False)


    return train_loader, val_loader, test_loader

# create DataLoader
#train_loader, val_loader, test_loader = create_dataloaders('./data/train_images', './data/test_images', './data/train_label.txt')


# for batch, feature in train_loader:
#     print(batch, feature)
#     break
'''
for batch in test_loader:
    feature_batch = batch
    print(feature_batch.size())
    break

for features, labels in train_loader:
    print(features.shape)  # torch.Size([32, 1, 64, 259])
    print(labels.shape)   # torch.Size([32])
'''



'''

import torch
from torch.utils.data import Dataset, DataLoader
import pickle
import os
from sklearn.model_selection import train_test_split
from torch.utils.data.dataloader import default_collate
import numpy as np


class PickleAudioDataset(Dataset):
    def __init__(self, file_paths, labels=None, transform = None): # transform = normalize_mel
        """
        file_paths: List of paths to the pickle files
        labels: Optional, pass labels for supervised learning
        transform: Optional, transformations to apply to the data
        """
        self.file_paths = file_paths
        self.labels = labels
        self.transform = transform

    def __len__(self):
        return len(self.file_paths)

    def __getitem__(self, idx):
        with open(self.file_paths[idx], 'rb') as f:
            feature = pickle.load(f)
            # print(111, type(feature))
        
        # Transform the feature if a transform is provided
        if self.transform:
            feature = self.transform(feature)
        
        # Convert numpy array to torch.Tensor and add a channel dimension
        feature = torch.tensor(feature, dtype=torch.float32).unsqueeze(0)  # Adds the channel dimension

        if self.labels is not None:
            label = self.labels[idx]
            return feature, label
        
        return feature
    
def normalize_mel(mel_spec):
    # <class 'data_loader.PickleAudioDataset'>
    epsilon = 1e-10  # smoothing
    mel_spec_mean = mel_spec.mean()
    mel_spec_std = mel_spec.std()
    
    # 确保标准差不为零
    if mel_spec_std == 0:
        mel_spec_std += epsilon
    
    mel_spec = (mel_spec - mel_spec_mean) / mel_spec_std
    return mel_spec

def load_labels(label_file):
    with open(label_file, "r") as f:
        return [int(label.strip()) for label in f]

def load_dataset_paths(root_dir, file_extension='.pkl'):
    files = [f for f in os.listdir(root_dir) if f.endswith(file_extension)]
    
    files_sorted = sorted(files, key=lambda x: int(x.split('.')[0]))           
    
      #  mel =  [os.path.join(root_dir, f) for f in sorted(os.listdir(root_dir)) if f.endswith(file_extension)]
      # only sort the length!!
    
    mel = [os.path.join(root_dir, f) for f in files_sorted]
    
    return mel


def create_dataloaders(train_dir, test_dir, label_file, batch_size=32, test_batch_size=32, val_size=0.2, shuffle=True):
    images_paths = load_dataset_paths(train_dir)
    labels = load_labels(label_file)
    # print(len(images_paths))  11886
    test_paths = load_dataset_paths(test_dir)

    train_paths, val_paths, train_labels, val_labels = train_test_split(images_paths, labels, test_size=val_size, random_state=42)

    train_dataset = PickleAudioDataset(train_paths, train_labels, normalize_mel)
    val_dataset = PickleAudioDataset(val_paths, val_labels, normalize_mel)
    # print(val_dataset[0][0].size())
    test_dataset = PickleAudioDataset(test_paths, None, normalize_mel)

    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
    test_loader = DataLoader(test_dataset, batch_size=test_batch_size, shuffle=False)

    


    return train_loader, val_loader, test_loader

# create DataLoader
#train_loader, val_loader, test_loader = create_dataloaders('./data/train_images', './data/test_images', './data/train_label.txt')


# for batch, feature in train_loader:
#     print(batch, feature)
#     break

for batch in test_loader:
    feature_batch = batch
    print(feature_batch.size())
    break

for features, labels in train_loader:
    print(features.shape)  # torch.Size([32, 1, 64, 259])
    print(labels.shape)   # torch.Size([32])

'''