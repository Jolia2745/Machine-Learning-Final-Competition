# srun --nodes=1 --cpus-per-task=4 --mem=32GB --time=12:00:00 --gres=gpu:1 --pty /bin/bash
print("start")
import os
import shutil
import time
...
class Args:
    def __init__(self):
        ...
        self.lr =  0.001   #0.01  # too big
        self.momentum = 0.9
        self.weight_decay = 1e-3  # 1e-2       
        self.print_freq = 20
        self.evaluate = False
...
def prediction(model, test_loader, epoch):
    model.eval()
    all_predictions = []
    ...
    if epoch:
        submission.to_csv(f'./submission_epoch_{epoch}.csv', index=False)
...
def main():
    ...
    train_loader, val_loader, test_loader = dl.create_dataloaders('../data_v2/train_images', '../data_v2/test_images', '../data_v2/train_label.txt',  batch_size=args.batch_size  , test_batch_size=args.batch_size)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            prediction(model, test_loader, epoch)
