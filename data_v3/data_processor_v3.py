'''
Data_processor for dataset version 3: extract multiple features from audio and calculate their means (Chroma STFT, mfcc, etc.)
'''


# calculate the meanimport os
import librosa
import pickle
import numpy as np
import os
...
def audio_to_image(file_path, save_path):
    ...
    features = np.hstack([mfccs_processed, mel_spec_processed, chroma_processed, spec_centroid_processed, spec_contrast_processed, tonnetz_processed])   # 2-D numpy arrays
    # print(features.shape) # (194,)
    features = features.reshape(-1, 1) # change it into 2-D numpy array
    # print(features.shape) # (194, 1)
